...
```{r LineMeanPerInt}
meanperint <- aggregate(activity$steps, by = list(activity$interval), mean, na.rm = TRUE)
...
for (i in 1:nrow(actfill)) 
	...
	actfill[i, "steps"] <- as.integer(meanperint$Steps[meanperint$Interval == actfill[i, "interval"]]) 
    } 
```

4. Make a histogram of the total number of steps taken each day and Calculate and report the mean and median total number of steps taken per day. 

```{r HistSumPerDayImputed}
...
```{r PanelPlotWeekdaysvsWeekends}
actweekdays <- actday[daytype == "Weekday", ]
weekdaymean <- aggregate(actweekdays$steps, by = list(actweekdays$interval), mean, na.rm = TRUE)
colnames(weekdaymean) <- c("Interval", "Steps")

actweekends <- actday[daytype == "Weekend", ]
weekendmean <- aggregate(actweekends$steps, by = list(actweekends$interval), mean, na.rm = TRUE)
